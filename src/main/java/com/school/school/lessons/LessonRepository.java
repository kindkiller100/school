...
public interface LessonRepository extends JpaRepository<Lesson, Long>, CustomRepository<Lesson> {
    Page<Lesson> findLessonsByStartDateTimeBetween(LocalDate dateFrom, LocalDate dateTo, Pageable pageable);
    ...
    long countDurationOfLessonsByTeacherInRange(long id, LocalDate dateFrom, LocalDate dateTo);

    @Query(value = "select sum(duration) from school_db.lessons where " +
        "school_db.lessons.startdatetime between ?1 and ?2 and " +
        "school_db.lessons.id in (select lesson_id from school_db.student_lesson " +
            "where school_db.student_lesson.is_visited and school_db.student_lesson.student_id = ?3)"
        , nativeQuery = true)
    long findDurationByStudentIdInRange(LocalDate dateFrom, LocalDate dateTo, long id);
